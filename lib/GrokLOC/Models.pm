...
Readonly::Scalar our $REPOSITORIES_TABLENAME => 'repositories';
...
our @EXPORT_OK =
  qw($STATUS_UNCONFIRMED $STATUS_ACTIVE $STATUS_INACTIVE $STATUS_ADMIN @STATUSES $RESPONSE_OK $RESPONSE_NOT_FOUND $RESPONSE_CONFLICT $RESPONSE_NO_ROWS $RESPONSE_ORG_ERR $RESPONSE_USER_ERR $NO_OWNER $REPOSITORIES_TABLENAME $ORGS_TABLENAME $USERS_TABLENAME safe_status);
our %EXPORT_TAGS = (
    all => [
        qw($STATUS_UNCONFIRMED $STATUS_ACTIVE $STATUS_INACTIVE $STATUS_ADMIN @STATUSES $RESPONSE_OK $RESPONSE_NOT_FOUND $RESPONSE_CONFLICT $RESPONSE_NO_ROWS $RESPONSE_ORG_ERR $RESPONSE_USER_ERR $NO_OWNER $REPOSITORIES_TABLENAME $ORGS_TABLENAME $USERS_TABLENAME safe_status)
    ],
